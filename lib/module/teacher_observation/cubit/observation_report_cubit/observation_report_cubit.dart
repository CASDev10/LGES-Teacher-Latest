...
class ObservationReportCubit extends Cubit<ObservationReportState> {
  ObservationReportCubit(this.repository) : super(ObservationReportState.initial());

  ObservationRepository repository;
  Future getObservationReport(String startDate, String endDate) async {
    emit(state.copyWith(
        observationReportStatus: ObservationReportStatus.loading));
    try {
      ObservationReportResponse observationReportResponse =
      await repository.getObservationReport(startDate, endDate);
      if (observationReportResponse.result == ApiResult.success) {
        emit(state.copyWith(
            observationReportStatus: ObservationReportStatus.success, observationReportList: observationReportResponse.data));
      } else {
        emit(state.copyWith(
